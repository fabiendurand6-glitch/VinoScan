...
const apiKey = 'AIzaSyClDDsccuRWaxBMQpMUNgII81AxlOxYclc'; // <-- COLlez VOTRE CLÉ ICI (ex: "AIzaSy...")
      ...
      return (
        <div className="flex flex-col items-center justify-center h-full p-6 text-center bg-slate-50">
          ...
          <p className="text-sm text-slate-600 mb-6">
            L'Intelligence Artificielle a renvoyé des données incompatibles.
          </p>
          <button
            onClick={() => {
...
const getGenericImageForType = (type) => {
  switch (type) {
    case 'BLANC':
      return 'https://images.unsplash.com/photo-1595914041793-11b0e00d7fb4?q=80&w=800&auto=format&fit=crop';
    case 'PETILLANT':
      return 'https://images.unsplash.com/photo-1599939571322-792a326cb6ae?q=80&w=800&auto=format&fit=crop';
    case 'ROSE':
      return 'https://images.unsplash.com/photo-1559596355-6bcfcc77112a?q=80&w=800&auto=format&fit=crop';
    default:
...
const normalizeData = (data) => {
  ...
  try {
    ...
    if (Array.isArray(data.accords_mets)) {
      safeAccordsMets = data.accords_mets
        .filter(Boolean)
    ...
    let safeTagsAccords = Array.isArray(data.tags_accords)
      ? data.tags_accords.filter(Boolean).map(safeString)
      : [];
    ...
    if (Array.isArray(data.comparateur)) {
      safeComparateur = data.comparateur.filter(Boolean).map((c) => {
        if (typeof c === 'object')
          return {
            site: safeString(c.site, 'Marchand'),
            prix: safeString(c.prix, '?'),
          };
    ...
    let accord_parfait = safeString(
      data.accord_parfait,
      safeAccordsMets.length > 0 ? safeAccordsMets[0] : '-'
    );

    let strToSearch = (
      safeString(data.type_simplifie) +
      ' ' +
      type
    ).toUpperCase();
    ...
    else if (strToSearch.includes('ROSE') || strToSearch.includes('ROSÉ'))
      type_simplifie = 'ROSE';
    else if (
      strToSearch.includes('CHAMPAGNE') ||
      strToSearch.includes('PETILLANT') ||
      strToSearch.includes('PÉTILLANT') ||
      strToSearch.includes('EFFERVESCENT') ||
      strToSearch.includes('CRÉMANT') ||
      strToSearch.includes('CREMANT') ||
      strToSearch.includes('BULLE')
    )
      type_simplifie = 'PETILLANT';

    let statut_apogee = safeString(data.statut_apogee, 'APOGEE');
    let prix_unitaire_nombre =
      Number(data.prix_unitaire_nombre) || extractPrice(data.prix_moyen) || 0;
    ...
    if (safeTagsAccords.length === 0 && safeAccordsMets.length > 0) {
      const accordsText =
        safeAccordsMets.join(' ').toLowerCase() +
        ' ' +
        accord_parfait.toLowerCase();
      Object.entries(keywords).forEach(([tag, words]) => {
        if (
          !safeTagsAccords.includes(tag) &&
          words.some((w) => accordsText.includes(w))
        ) {
          safeTagsAccords.push(tag);
        }
      });
      if (safeTagsAccords.length === 0) {
        if (type_simplifie === 'ROUGE')
          safeTagsAccords.push('VIANDE_ROUGE', 'FROMAGE');
        if (type_simplifie === 'BLANC')
          safeTagsAccords.push('POISSON', 'VIANDE_BLANCHE', 'APERITIF');
        if (type_simplifie === 'PETILLANT')
          safeTagsAccords.push('APERITIF', 'DESSERT');
        if (type_simplifie === 'ROSE')
          safeTagsAccords.push('APERITIF', 'POISSON');
      }
    }
    ...
    return {
      ...
      declin,
      accord_parfait,
      accords_mets: safeAccordsMets,
      tags_accords: safeTagsAccords,
      comparateur: safeComparateur,
    };
  } catch (e) {
    return {
      nom: "Erreur d'analyse",
      type_simplifie: 'AUTRE',
      accords_mets: [],
      tags_accords: [],
      comparateur: [],
    };
...
export default function App() {
      ...
      (snapshot) => {
        ...
        snapshot.forEach((doc) => {
          ...
          scans.push({
            ...
            dateStr: String(
              itemData.dateStr || new Date().toLocaleDateString('fr-FR')
            ),
  ...
  const startCamera = async () => {
    try {
      ...
      setErrorMsg(
        "Impossible d'accéder à la caméra. Vérifiez vos autorisations."
      );
      setView('error');
    }
  };
  ...
  const processAIResult = async (
    ...
    let finalImage =
      sourceImage ||
      defaultImageFallback ||
      getGenericImageForType(parsedData.type_simplifie);
  ...
  const fetchAIRecommendation = async (type, apogee, food, price) => {
    ...
    try {
      const mapType = {
        ALL: 'Peu importe le type',
        ROUGE: 'Vin Rouge',
        BLANC: 'Vin Blanc',
        ROSE: 'Vin Rosé',
        PETILLANT: 'Champagne ou Pétillant',
      };
      const mapApogee = {
        ALL: 'Peu importe',
        A_GARDER: 'Jeune',
        APOGEE: 'Apogée',
        DECLIN: 'Vieux millésime',
      };
      const mapFood = {
        ALL: 'Peu importe',
      ...
      const mapPrice = {
        ALL: 'Différentes tranches',
        BUDGET: '< 20 euros',
        MEDIUM: '20 - 50 euros',
        PREMIUM: '> 50 euros',
      };

      const prompt = `Tu es un sommelier de classe mondiale. Recommandations : Type: ${
        mapType[type]
      }, Repas: ${mapFood[food]}, Apogée: ${mapApogee[apogee]}, Budget: ${
        mapPrice[price]
      }.
  ...
  const processRecommendationSelection = async (wineData) => {
    await processAIResult(
      ...
      null,
      getGenericImageForType(wineData.type_simplifie),
      true
    );
  };
  ...
  const handleShare = async (wine) => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: 'Mon vin VinoScan', text: shareText });
      } catch (e) {}
    } else {
  ...
  const executeAction = async () => {
    ...
    if (newStock === 0 && !newInHistory && !currentScanObj.wishlist) {
      ...
      setScanHistory((prev) =>
        prev.map((item) =>
          item.id === id
            ? { ...item, stock: newStock, in_history: newInHistory }
            : item
        )
      );
  ...
  const updateStock = async (scanId, currentStock, change) => {
    ...
    setScanHistory((prev) =>
      prev.map((item) =>
        item.id === scanId ? { ...item, stock: newStock } : item
      )
    );
    ...
    if (
      ...
      try {
        ...
        const inHistory = currentScanObj
          ? currentScanObj.in_history !== false
          : true;
  ...
  const NavigationBar = () => (
    <div className="absolute bottom-0 w-full bg-white border-t border-slate-200 flex justify-around items-center pb-safe pt-2 px-1 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] z-20 h-16">
      ...
      <button
        ...
        className={`flex flex-col items-center justify-center w-full h-full space-y-1 ${
          view === 'recommendation' || view === 'recommendationList'
            ? 'text-amber-600'
            : 'text-slate-400 hover:text-slate-600'
        }`}
      >
    ...
    <div className="flex flex-col items-center justify-center h-full p-6 space-y-8 pb-20">
      <div className="text-center space-y-4">
        ...
        <p className="text-slate-500 max-w-sm mx-auto text-sm">
          Gérez votre cave, analysez vos vins et découvrez leur apogée parfaite.
        </p>
      </div>
      <div className="w-full max-w-sm space-y-3 pt-4">
        <button
          onClick={startCamera}
          className="w-full flex items-center justify-center space-x-3 bg-rose-900 hover:bg-rose-800 text-white p-4 rounded-2xl shadow-lg transition-all active:scale-95"
        >
          <Camera className="w-6 h-6" />
          <span className="font-medium text-lg">Scanner une bouteille</span>
        </button>
        <div className="flex space-x-3">
          <label className="flex-1 flex flex-col items-center justify-center space-y-2 bg-white border-2 border-slate-200 text-slate-700 hover:bg-slate-50 p-4 rounded-2xl cursor-pointer transition-all active:scale-95 shadow-sm">
            ...
            <span className="font-medium text-sm">Galerie photo</span>
            <input
              type="file"
              accept="image/*"
              className="hidden"
          ...
          <button
            onClick={() => setView('manualSearch')}
            className="flex-1 flex flex-col items-center justify-center space-y-2 bg-slate-900 text-white hover:bg-slate-800 p-4 rounded-2xl shadow-sm transition-all active:scale-95"
          >
            <Search className="w-6 h-6" />
            <span className="font-medium text-sm">Recherche texte</span>
          </button>
        </div>
      </div>
    </div>
  ...
  const ManualSearchView = () => {
    ...
    const handleSearch = (e) => {
      e.preventDefault();
      if (query.trim()) searchWineText(query);
    };
    return (
      <div className="flex flex-col h-full bg-slate-50 pb-20">
        <div className="bg-white pt-12 pb-4 px-6 shadow-sm z-10 sticky top-0 flex items-center">
          <button
            onClick={() => setView('home')}
            className="mr-4 p-2 bg-slate-100 text-slate-600 rounded-full hover:bg-slate-200"
          >
          ...
          <div>
            ...
            <p className="text-slate-500 text-xs mt-1">
              Recherche dans la base de données mondiale
            </p>
          </div>
        </div>
        <div className="p-6">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-4 py-4 bg-white border border-slate-200 rounded-2xl focus:ring-2 focus:ring-rose-500 focus:border-transparent outline-none shadow-sm text-lg"
                />
              </div>
            </div>
            <button
              type="submit"
              disabled={!query.trim()}
              className="w-full py-4 bg-slate-900 text-white rounded-xl font-medium shadow-lg hover:bg-slate-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed mt-2"
            >
              Rechercher ce vin
            </button>
          </form>
  ...
  const CellarView = () => {
    ...
    const filteredItems = useMemo(() => {
      return cellarItems.filter((item) => {
        const matchType =
          filterType === 'ALL' || item.data.type_simplifie === filterType;
        const matchApogee =
          filterApogee === 'ALL' || item.data.statut_apogee === filterApogee;
    ...
    const totalValue = filteredItems.reduce(
      (acc, curr) =>
        acc +
        (curr.data.prix_unitaire_nombre || 0) *
          (cellarTab === 'STOCK' ? parseInt(curr.stock) || 0 : 1),
      0
    );

    const declinAlerts =
      cellarTab === 'STOCK'
        ? filteredItems.filter((i) => i.data.statut_apogee === 'DECLIN')
        : [];
    ...
    const getApogeeBadge = (statut) => {
      switch (statut) {
        case 'A_GARDER':
          return (
            <div className="flex items-center space-x-1 text-xs text-indigo-600 bg-indigo-50 px-2 py-1 rounded">
          ...
          <div className="flex justify-between items-end mb-4">
            <div>
              ...
              <p className="text-slate-500 text-sm mt-1">
                {totalBottles}{' '}
                {cellarTab === 'STOCK' ? 'bouteilles' : 'souhaits'}
              </p>
            </div>
            <div className="text-right">
              ...
              <div className="flex items-center justify-end space-x-1 text-slate-900">
          ...
          <div className="space-y-3 relative">
            ...
            {cellarTab === 'STOCK' && (
              <div className="flex items-center space-x-2 overflow-x-auto pb-1 scrollbar-hide">
                ...
                <button
                  onClick={() => setFilterApogee('A_GARDER')}
                  className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-colors ${
                    filterApogee === 'A_GARDER'
                ...
                <button
                  onClick={() => setFilterApogee('APOGEE')}
                  className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-colors ${
                    filterApogee === 'APOGEE'
                ...
                <button
                  onClick={() => setFilterApogee('DECLIN')}
                  className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-colors ${
                    filterApogee === 'DECLIN'
        ...
        <div className="flex-1 overflow-y-auto p-4">
            ...
            <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-start space-x-3 mb-4 animate-in fade-in">
              ...
              <div>
                ...
                <p className="text-sm text-red-600">
                  Vous avez {declinAlerts.length} bouteille(s) sur le déclin. Ne
                  tardez plus pour les déguster !
                </p>
              </div>
            </div>
          )}
          ...
          {filteredItems.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center p-6 opacity-50 mt-10">
              {cellarTab === 'STOCK' ? (
                <Archive className="w-16 h-16 mb-4 text-slate-400" />
              ) : (
                <Heart className="w-16 h-16 mb-4 text-slate-400" />
              )}
            ...
            <div className="space-y-4">
              {filteredItems.map((item) => (
                ...
                >
                  <div
                    onClick={() => openExistingWine(item, 'cellar')}
                    className="p-4 flex items-start space-x-4 active:bg-slate-50 cursor-pointer"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start mb-1">
                        <span className="text-[10px] font-bold uppercase tracking-wider text-rose-700 bg-rose-50 px-2 py-0.5 rounded">
                          {item.data.type_simplifie === 'PETILLANT'
                            ? 'Pétillant'
                            : item.data.type_simplifie}
                        </span>
                        <span className="text-xs font-bold text-slate-400">
                          {item.data.annee}
                        </span>
                      </div>
                      <h3 className="font-serif text-slate-900 truncate font-semibold leading-tight mb-1">
                        {item.data.nom}
                      </h3>
                      {item.location && (
                  ...
                  {cellarTab === 'STOCK' ? (
                    <div className="bg-slate-50 border-t border-slate-100 p-3 flex justify-between items-center">
                      ...
                      <div className="flex items-center space-x-3 bg-white border border-slate-200 rounded-full px-2 py-1 shadow-sm">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            handleDirectStockChange(
                              item.id,
                              Math.max(0, item.stock - 1)
                            );
                          }}
                          className="w-8 h-8 flex items-center justify-center text-slate-500 hover:text-rose-600 hover:bg-rose-50 rounded-full transition-colors"
                        >
                        ...
                        <input
                          ...
                          value={item.stock}
                          onChange={(e) =>
                            handleDirectStockChange(item.id, e.target.value)
                          }
                          onBlur={(e) => {
                            if (e.target.value === '')
                              handleDirectStockChange(item.id, '0');
                          }}
                          className="font-bold text-lg w-10 text-center text-slate-800 bg-slate-100/50 outline-none focus:bg-slate-200 rounded [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                        />
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            handleDirectStockChange(item.id, item.stock + 1);
                          }}
                          className="w-8 h-8 flex items-center justify-center text-slate-500 hover:text-emerald-600 hover:bg-emerald-50 rounded-full transition-colors"
                        >
                    ...
                    <div className="bg-slate-50 border-t border-slate-100 p-3 flex justify-end">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          genericUpdate(item.id, { wishlist: false, stock: 1 });
                          showToast('Ajouté à la cave !');
                        }}
                        className="px-4 py-2 bg-slate-900 text-white text-sm font-medium rounded-lg shadow-sm hover:bg-slate-800"
                      >
                        J'ai acheté ce vin
                      </button>
                    </div>
            ...
            <div className="space-y-6">
              {Object.entries(groupedByLocation).map(([shelfName, bottles]) => (
                <div
                  key={shelfName}
                  className="bg-slate-800 rounded-xl overflow-hidden shadow-[0_10px_15px_-3px_rgba(0,0,0,0.3)] border-b-8 border-amber-900/80"
                >
                  <div className="bg-slate-900 px-4 py-3 text-amber-50 font-serif font-medium flex justify-between items-center border-b border-slate-700/50">
                    <span className="flex items-center">
                      <MapPin className="w-4 h-4 mr-2 opacity-70" /> {shelfName}
                    </span>
                    <span className="text-xs bg-slate-800 px-2 py-1 rounded uppercase tracking-wider">
                      {bottles.length} bot.
                    </span>
                  </div>
                  <div className="p-4 flex flex-wrap gap-x-4 gap-y-6 justify-center bg-gradient-to-b from-slate-800 to-slate-900 min-h-[140px] items-end pb-2">
                    {bottles.map((bottle) => (
                      <div
                        key={bottle.id}
                        onClick={() => openExistingWine(bottle, 'cellar')}
                      ...
                      >
                        <div className="w-12 h-32 bg-slate-950 rounded-t-xl rounded-b-sm overflow-hidden border-2 border-slate-700/50 relative shadow-2xl">
                          ...
                          />
                          <div
                            className={`absolute bottom-0 left-0 w-full h-2 ${getColorForType(
                        ...
                        <p className="text-[9px] text-center text-slate-300 mt-2 truncate w-14 mx-auto">
                          {String(bottle.data?.nom || 'Vin').split(' ')[0]}
                        </p>
                      </div>
                    ))}
                  </div>
  ...
  const HistoryView = () => {
    const historyItems = scanHistory.filter(
      (item) => item.in_history !== false
    );
    ...
    return (
      <div className="flex flex-col h-full bg-slate-50 pb-20">
        <div className="bg-white pt-12 pb-4 px-6 shadow-sm z-10 sticky top-0">
          ...
          <p className="text-slate-500 text-xs mt-1">
            Toutes les bouteilles analysées
          </p>
        </div>
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {historyItems.length === 0 ? (
            ...
            historyItems.map((item) => (
              <div
                key={item.id}
                onClick={() => openExistingWine(item, 'history')}
                className="bg-white rounded-xl p-3 shadow-sm border border-slate-100 flex items-center space-x-4 active:scale-[0.98] transition-transform cursor-pointer relative overflow-hidden"
              >
                ...
                <div className="flex-1 min-w-0 pr-4">
                  ...
                  <h3 className="font-serif text-slate-900 truncate font-medium text-sm">
                    {item.data.nom}
                  </h3>
                </div>
  ...
  // =========================================================================
  // LA NOUVELLE VUE COMPTE (AVEC CONNEXION)
  // =========================================================================
  const AccountView = () => {
    ...
    const handleAuth = async (e) => {
      ...
      try {
        ...
        setAuthError(
          'Erreur : Vérifiez vos identifiants ou le mot de passe (min 6 caractères).'
        );
      }
    };
    ...
    // Si l'utilisateur n'est pas connecté avec un vrai compte
        ...
        <div className="flex flex-col h-full bg-slate-50 pb-20 overflow-y-auto p-6">
          <div className="flex-1 flex flex-col items-center justify-center max-w-sm mx-auto w-full">
            ...
            <h2 className="text-2xl font-serif text-slate-900 mb-2">
              Sauvegardez votre cave
            </h2>
            <p className="text-sm text-slate-500 text-center mb-8">
              Créez un compte gratuitement pour retrouver votre cave sur tous
              vos appareils.
            </p>

            {authError && (
              <div className="bg-red-50 text-red-600 text-sm p-3 rounded-lg w-full mb-4 text-center">
                {authError}
              </div>
            )}
            ...
            <form onSubmit={handleAuth} className="w-full space-y-4">
              <input
                ...
                className="w-full p-4 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-emerald-200"
                required
              />
              <input
                type="password"
              ...
              <button
                type="submit"
                className="w-full py-4 bg-slate-900 text-white rounded-xl font-medium shadow-lg hover:bg-slate-800 transition-colors"
              >
            ...
            <button
              ...
              className="mt-6 text-slate-500 text-sm hover:text-slate-700 underline underline-offset-4"
            >
              {authMode === 'login'
    ...
    const totalBottles = itemsInStock.reduce(
      (acc, curr) => acc + (curr.stock || 0),
      0
    );
    const totalValue = itemsInStock.reduce(
      (acc, curr) =>
        acc + (curr.data.prix_unitaire_nombre || 0) * (curr.stock || 0),
      0
    );

    const countType = (type) =>
      itemsInStock
        .filter((i) => i.data.type_simplifie === type)
    ...
    const getPct = (val) =>
      totalBottles === 0 ? 0 : Math.round((val / totalBottles) * 100);
    ...
    return (
      <div className="flex flex-col h-full bg-slate-50 pb-20 overflow-y-auto">
        ...
        <div className="p-6 flex flex-col items-center text-center">
          ...
          <p className="text-xs text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full mt-2 font-medium">
          ...
          <div className="w-full bg-white rounded-2xl p-5 border border-slate-100 shadow-sm mt-8 mb-6">
            <h3 className="font-semibold text-slate-800 flex items-center mb-4">
              <PieChart className="w-5 h-5 mr-2 text-indigo-600" /> Mon
              Dashboard
            </h3>
            ...
            <div className="h-4 w-full flex rounded-full overflow-hidden mb-3">
              {red > 0 && (
                <div
                  style={{ width: `${getPct(red)}%` }}
                  className="bg-rose-800 h-full"
                ></div>
              )}
              {white > 0 && (
                <div
                  style={{ width: `${getPct(white)}%` }}
                  className="bg-amber-100 h-full"
                ></div>
              )}
              {rose > 0 && (
                <div
                  style={{ width: `${getPct(rose)}%` }}
                  className="bg-pink-300 h-full"
                ></div>
              )}
              {spark > 0 && (
                <div
                  style={{ width: `${getPct(spark)}%` }}
            ...
            <div className="grid grid-cols-2 gap-y-2 text-sm">
              <div className="flex items-center">
                ...
                <span className="text-slate-600">Rouges ({getPct(red)}%)</span>
              </div>
              <div className="flex items-center">
                <div className="w-3 h-3 rounded-full bg-amber-100 mr-2 border border-slate-200"></div>
              ...
              <div className="flex items-center">
                ...
                <span className="text-slate-600">Rosés ({getPct(rose)}%)</span>
              </div>
              <div className="flex items-center">
          ...
          <button
            ...
            className="w-full flex items-center justify-center space-x-2 py-4 bg-slate-200 text-slate-700 rounded-xl font-medium hover:bg-slate-300 transition-colors"
          >
  ...
  const CameraView = () => (
    <div className="relative h-full w-full bg-black flex flex-col">
      <button
        ...
        className="absolute top-6 left-6 z-10 p-3 bg-black/50 backdrop-blur text-white rounded-full"
      >
        ...
        <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
          <div className="w-3/4 h-1/2 border-2 border-white/50 rounded-2xl flex flex-col justify-between p-4">
            <div className="flex justify-between">
              <div className="w-8 h-8 border-t-4 border-l-4 border-rose-500 rounded-tl-lg"></div>
              <div className="w-8 h-8 border-t-4 border-r-4 border-rose-500 rounded-tr-lg"></div>
            </div>
            <ScanLine className="w-12 h-12 text-white/30 self-center animate-pulse" />
            <div className="flex justify-between">
              <div className="w-8 h-8 border-b-4 border-l-4 border-rose-500 rounded-bl-lg"></div>
              <div className="w-8 h-8 border-b-4 border-r-4 border-rose-500 rounded-br-lg"></div>
            </div>
          </div>
        </div>
      </div>
      <div className="h-32 bg-black pb-8 pt-4 flex items-center justify-center">
        <button
          ...
          className="w-20 h-20 bg-white rounded-full border-4 border-slate-300 flex items-center justify-center shadow-[0_0_20px_rgba(255,255,255,0.3)] active:scale-90 transition-transform"
        >
  ...
  const AnalyzingView = () => (
    <div className="flex flex-col items-center justify-center h-full p-6 bg-slate-50">
      ...
      <p className="text-slate-500 text-sm mt-2 text-center">
        Notre sommelier travaille sur votre demande...
      </p>
    </div>
  );
  ...
  const ErrorView = () => (
    <div className="flex flex-col items-center justify-center h-full p-6 text-center">
      ...
      <button
        onClick={goBack}
        className="px-8 py-3 bg-slate-800 text-white rounded-full font-medium hover:bg-slate-700 transition-colors"
      >
        Retour
      </button>
    </div>
  ...
  const RecommendationView = () => {
    ...
    const handleRecommend = () => {
      fetchAIRecommendation(filterType, filterApogee, filterFood, filterPrice);
    };
        ...
        <div className="p-6 space-y-8">
          ...
          <div className="space-y-3">
            <h3 className="font-semibold text-slate-800 flex items-center space-x-2">
              ...
              <span>Qu'allez-vous manger ?</span>
            </h3>
            <div className="flex flex-wrap gap-2">
              <button
              ...
              <button
                onClick={() => setFilterFood('APERITIF')}
                className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${
                  filterFood === 'APERITIF'
                    ? 'bg-amber-600 text-white shadow-md'
                    : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'
              ...
              <button
                onClick={() => setFilterFood('VIANDE_ROUGE')}
                className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${
                  filterFood === 'VIANDE_ROUGE'
                    ? 'bg-amber-600 text-white shadow-md'
                    : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'
                }`}
              >
              ...
              <button
                onClick={() => setFilterFood('VIANDE_BLANCHE')}
                className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${
                  filterFood === 'VIANDE_BLANCHE'
                    ? 'bg-amber-600 text-white shadow-md'
                    : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'
                }`}
              >
          ...
          <div className="space-y-3">
            <h3 className="font-semibold text-slate-800 flex items-center space-x-2">
              <Wine className="w-5 h-5 text-rose-800" />
              <span>Quel type de vin ?</span>
            </h3>
            <div className="flex flex-wrap gap-2">
              {['ALL', 'ROUGE', 'BLANC', 'PETILLANT', 'ROSE'].map((type) => (
                <button
          ...
          <button
            ...
            className="w-full py-4 bg-amber-600 text-white rounded-xl font-medium shadow-lg shadow-amber-600/30 hover:bg-amber-700 transition-all active:scale-95 flex justify-center items-center space-x-2 mt-4"
          >
            <Sparkles className="w-5 h-5" />
            <span>Me conseiller un vin</span>
          </button>
        </div>
      </div>
    );
      ...
      <div className="flex flex-col h-full bg-amber-50/30 pb-20">
        <div className="bg-white pt-12 pb-4 px-6 shadow-sm z-10 sticky top-0 flex items-center justify-between border-b border-amber-100">
          <div className="flex items-center">
            <button
              onClick={() => setView('recommendation')}
              className="mr-3 p-2 bg-slate-100 text-slate-600 rounded-full hover:bg-slate-200"
            >
              <ChevronLeft className="w-5 h-5" />
            </button>
            <div>
              ...
              <p className="text-slate-500 text-xs mt-1">
                {sortedList.length} vins trouvés pour vous
              </p>
            </div>
          </div>
          <button
            ...
            className="flex items-center space-x-2 bg-amber-100 text-amber-700 px-3 py-2 rounded-xl text-sm font-medium hover:bg-amber-200 transition-colors"
          >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {sortedList.map((wine, index) => {
            ...
            return (
              <div
                key={index}
                className="bg-white rounded-2xl p-4 shadow-sm border border-slate-200 flex space-x-4"
              >
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-start mb-1">
                      <span className="text-[10px] font-bold uppercase tracking-wider text-rose-700 bg-rose-50 px-2 py-0.5 rounded">
                        {wine.type_simplifie === 'PETILLANT'
                          ? 'Pétillant'
                          : wine.type_simplifie || 'VIN'}
                      </span>
                      <div className="flex items-end space-x-1 text-emerald-700 font-bold">
                        <span className="text-xl leading-none">
                          {wine.prix_unitaire_nombre || '?'}
                        </span>
                        <span className="text-sm">€</span>
                      </div>
                    </div>
                    <h3 className="font-serif text-slate-900 text-base leading-tight mb-1 line-clamp-2">
                      {wine.nom}
                    </h3>
                  ...
                  <button
                    onClick={() => processRecommendationSelection(wine)}
                    className="w-full py-2 bg-slate-900 text-white rounded-xl text-xs font-medium shadow-md hover:bg-slate-800 transition-colors"
                  >
    ...
    return (
      <div className="flex flex-col h-full bg-slate-50 overflow-y-auto pb-8">
        <div className="relative h-64 bg-slate-800 overflow-hidden shrink-0">
          <img
            src={imageSrc}
            alt="Scanned bottle blur"
            className="w-full h-full object-cover opacity-50 blur-sm"
          />
          <div className="absolute inset-0 flex items-center justify-center p-4">
            <img
              src={imageSrc}
              alt="Scanned bottle clear"
              className="max-h-full rounded-lg shadow-2xl"
            />
          </div>
          <button
            onClick={goBack}
            className="absolute top-4 left-4 p-2 bg-white/20 backdrop-blur text-white rounded-full hover:bg-white/30 transition-colors z-20"
          >
            <ChevronLeft className="w-6 h-6" />
          </button>
          ...
          {currentScanObj && (
            <button
              onClick={() => handleShare(currentScanObj)}
              className="absolute top-4 right-4 p-2 bg-white/20 backdrop-blur text-white rounded-full hover:bg-white/30 transition-colors z-20"
            >
        ...
        <div className="px-4 -mt-6 relative z-10">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 mb-4">
            <div className="flex justify-between items-start mb-2">
              <span className="text-xs font-bold uppercase tracking-wider text-rose-700 bg-rose-50 px-2 py-1 rounded-md">
                {type}
              </span>
            ...
            <h2 className="text-2xl font-serif text-slate-900 leading-tight mb-2">
              {nom}
            </h2>
            ...
            <div className="flex items-center text-lg text-slate-500 font-medium mb-4">
              ...
              <div className="relative flex items-center ml-2">
                <input
                  ...
                  onBlur={() =>
                    currentScanId &&
                    updateDataField(currentScanId, 'annee', tempAnnee)
                  }
          ...
          {currentScanObj && (
            <div className="bg-slate-900 rounded-2xl shadow-lg p-6 mb-4 text-white">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="font-medium text-slate-200">Dans ma cave</h3>
                </div>
                <div className="flex items-center space-x-3 bg-slate-800 rounded-full p-1">
                  <button
                    ...
                    className="w-12 h-12 flex items-center justify-center hover:bg-slate-700 rounded-full transition-colors"
                  >
                  ...
                  <input
                    ...
                    value={stock}
                    onChange={(e) =>
                      handleDirectStockChange(currentScanObj.id, e.target.value)
                    }
                    onBlur={(e) => {
                      if (e.target.value === '')
                        handleDirectStockChange(currentScanObj.id, '0');
                    }}
                    className="w-14 h-12 text-center text-2xl font-bold bg-slate-700/50 rounded-xl text-white outline-none focus:bg-slate-700 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                  />
                  <button
                    onClick={() =>
                      handleDirectStockChange(currentScanObj.id, stock + 1)
                    }
                    className="w-12 h-12 flex items-center justify-center bg-rose-600 hover:bg-rose-500 rounded-full transition-colors shadow-lg shadow-rose-900/50"
                  >
              ...
              {stock === 0 ? (
                <button
                  onClick={() =>
                    genericUpdate(currentScanObj.id, { wishlist: !isWishlist })
                  }
                  className={`w-full py-3 rounded-xl font-medium flex items-center justify-center space-x-2 transition-colors border ${
                    isWishlist
                      ? 'bg-pink-900/40 border-pink-800 text-pink-200 hover:bg-pink-900/60'
                ...
                <div className="space-y-2 mt-4 pt-4 border-t border-slate-800">
                  <label className="text-xs font-medium text-slate-400 uppercase tracking-wider flex items-center">
                    <MapPin className="w-3 h-3 mr-1" /> Emplacement en cave
                  </label>
                  <input
                    type="text"
                    value={tempLocation}
                    ...
                    onBlur={() =>
                      genericUpdate(currentScanObj.id, {
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 mb-4">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center space-x-2">
                  <Edit3 className="w-5 h-5 text-rose-800" />
                ...
                <div className="flex space-x-1">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button
                      key={star}
                      onClick={() =>
                        genericUpdate(currentScanObj.id, { rating: star })
                      }
                    >
                      <Star
                        className={`w-6 h-6 ${
              ...
              <textarea
                ...
                onBlur={() =>
                  genericUpdate(currentScanObj.id, { notes: tempNotes })
                }
          ...
          <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl shadow-sm border border-amber-100 p-6 mb-4">
            ...
            <p className="text-amber-800 font-medium text-lg leading-snug">
              {accord_parfait}
            </p>

            {safeAccordsMets.length > 0 && (
              <div className="mt-5 pt-4 border-t border-amber-200/50">
                <h4 className="text-xs font-bold uppercase tracking-wider text-amber-800/70 mb-3">
                  Autres excellents choix :
                </h4>
                <ul className="space-y-2">
                  {safeAccordsMets.map((plat, index) => (
                    ...
                    >
                      <div className="w-1 h-1 rounded-full bg-amber-500 mt-2 shrink-0"></div>
                      <span className="text-amber-900/80">{plat}</span>
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <div className="space-y-4">
              <div className="flex items-start">
                ...
                <div className="-mt-1.5 pb-6">
                  ...
                  <p className="text-sm text-slate-500">{potentiel_garde}</p>
                </div>
              </div>
              <div className="flex items-start">
                <div className="w-10 flex flex-col items-center">
              ...
              <div className="flex items-start">
                ...
                <div className="-mt-1">
                  ...
                  <p className="text-sm text-slate-600">{declin}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 mb-6">
            ...
            <div className="flex items-end space-x-1 mb-6">
              ...
              <span className="text-slate-500 mb-1 ml-2">
                estimé / bouteille
              </span>
            </div>
          ...
          <div className="flex flex-col space-y-3 mt-6">
            {currentScanObj && currentScanObj.in_history !== false && (
              <button
                onClick={() =>
                  setScanAction({ id: currentScanObj.id, type: 'history' })
                }
                className="w-full flex items-center justify-center space-x-2 py-4 bg-slate-100 text-slate-700 rounded-xl font-medium shadow-sm hover:bg-slate-200 transition-colors border border-slate-200"
              >
            ...
            {currentScanObj && currentScanObj.stock > 0 && (
              <button
                onClick={() =>
                  setScanAction({ id: currentScanObj.id, type: 'cellar' })
                }
                className="w-full flex items-center justify-center space-x-2 py-4 bg-red-50 text-red-600 rounded-xl font-medium shadow-sm hover:bg-red-100 transition-colors border border-red-100"
              >
                <Archive className="w-5 h-5" />
                <span>Retirer de ma cave</span>
              </button>
            )}
          </div>
        </div>
    ...
    <ErrorBoundary onReset={() => setView('home')}>
      <div className="w-full max-w-md mx-auto h-[100dvh] bg-white sm:border-x sm:border-slate-200 overflow-hidden relative shadow-2xl text-slate-900 font-sans">
        ...
        {(view === 'home' ||
          ...
          view === 'account' ||
          view === 'recommendation' ||
          view === 'recommendationList') && <NavigationBar />}
        ...
        {scanAction && (
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
            <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl animate-in zoom-in-95 duration-200">
              ...
              <h3 className="text-xl font-serif text-center text-slate-900 mb-2">
                {scanAction.type === 'history'
                  ? "Retirer de l'historique ?"
                  : 'Retirer de la cave ?'}
              </h3>
              <p className="text-slate-600 text-sm text-center mb-6">
                {scanAction.type === 'history'
                  ? "Ce vin n'apparaîtra plus dans votre historique de scans."
                  : "Le stock de ce vin passera à 0 et il n'apparaîtra plus dans votre cave."}
              </p>
              <div className="flex space-x-3">
                <button
                  onClick={() => setScanAction(null)}
